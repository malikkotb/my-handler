...
This page documents **in-page motion** gated by **`useContentReady()`** (view transitions): **`AnimatedText`** (line-split reveals), Portable Text wrappers in **`features/rich-text`**, and how **draft mode** affects animation.

## View-transition gate

**`AnimatedText`** uses **`useContentReady()`** from `features/use-content-ready.ts`. During a view transition (`isViewTransitionComplete === false`), animations are held so they don't play under the old snapshot. When no `ViewTransitionProvider` wraps the tree, the gate defaults to ready immediately.
...
- **`AnimatedSanityRichText`:** same body, wrapped in **`AnimatedText`** with **`flex flex-col gap-[1em]`**; line-split and motion follow **`AnimatedText`** (draft / reduced motion / **`useContentReady()`**). Pass **`AnimatedText`** props (e.g. **`animationDelay`**, **`revert`**, **`viewport`**).
