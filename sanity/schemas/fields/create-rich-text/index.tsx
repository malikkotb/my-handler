...
import { buildMediaPreview, createMediaField } from "../create-media";
...
const inlineMediaField = defineArrayMember({
  ...
  preview: {
    select: {
      ...
      thumbTime: "media.video.asset.thumbTime",
      videoCoverUrl: "media.videoCover.asset.url",
    },
    prepare: (props) => {
...
export function createRichTextField({
  ...
  // Filter blocks based on whitelist and blacklist.
  // Note: this will also remove blocks that have no name.
  const enabledBlocks = blocks.filter((schema) => {
