...
import { LottieFileInput } from "../../inputs/lottie-file-input";
import { RiveFileInput } from "../../inputs/rive-file-input";
import { composeValidation, requiredIf, visibleIf } from "../../utils";
...
function mediaTypeValidation(R: Rule) {
  return R.custom((value) => {
    if (!value || typeof value !== "object") {
      return true;
    }

    return (value as { type?: string }).type ? true : "Select a media type.";
  });
}
...
function createMediaField({
  ...
  if (whitelist?.length && blacklist?.length) {
    throw new Error("createMediaField: Cannot provide both whitelist and blacklist parameters.");
  }

  const enabledTypeOptions = MEDIA_TYPE_OPTIONS.filter((option) => {
    if (whitelist?.length) {
      return whitelist.includes(option.value);
    }

    if (blacklist?.length) {
      return !blacklist.includes(option.value);
    }
  ...
  return defineField({
    ...
    fields: [
      ...
      defineField({
        ...
        components: {
          input: (props) => <RiveFileInput {...props} />,
        },
        validation: (R) =>
          R.custom((val, { parent }) => {
      ...
      defineField({
        ...
        components: {
          input: (props) => <LottieFileInput {...props} />,
        },
        validation: (R) =>
          R.custom((val, { parent }) => {
            const t = (parent as { type?: string } | undefined)?.type;
...
export { buildMediaPreview, createMediaField };
