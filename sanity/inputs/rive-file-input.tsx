...
function RiveFileInput(props: FileInputProps) {
  ...
  const dimensionsPath = React.useMemo(() => parentPath.concat("riveDimensions"), [parentPath]);
  const dimensionsPatchPath = React.useMemo(() => toPatchPath(dimensionsPath), [dimensionsPath]);

  const riveDimensions = useFormValue(dimensionsPath);
  const riveDimensionsRef = React.useRef<unknown>(riveDimensions);
  riveDimensionsRef.current = riveDimensions;
  ...
  const unsetDimensionsIfPresent = React.useCallback(async () => {
    if (riveDimensionsRef.current == null) {
      return;
    }
  ...
  const readDimensions = React.useCallback(async () => {
    if (!ref) {
      setStatus("error");
      setMessage("Upload a Rive file first.");
      return;
    }
    ...
    try {
      ...
      const dim = await parseRiveAssetDimensionsFromBuffer(buffer);

      if (!dim) {
        throw new Error("Could not read width/height from this .riv file.");
      }
      ...
      setStatus("error");
      setMessage(e instanceof Error ? e.message : "Invalid Rive file");
    }
