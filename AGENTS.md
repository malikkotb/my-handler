...
- **accessibility**: Keyboard, focus, semantic HTML, ARIA/names, forms/errors, contrast, reduced motion; see `.agents/skills/accessibility/`.
...
- **code-style**: TypeScript/React readability—`type` over `interface`, React 19 `ref` as a prop, braced `if/else` and spacing around conditionals; **`run()`** from `~/features/utils/common` for heavy conditional JSX and scoped Server Component logic (JSX-friendly IIFE; see the **`run()`** section in this skill).
- **docs-maintenance**: Keep `docs/` and the root README accurate when code behavior, env vars, setup steps, or workflows change.
- **view-transitions**: App Router view transitions (`app-view-transitions.tsx`), `ViewTransitionProvider`, Link/navigation timing, content gating.
