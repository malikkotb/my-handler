...
- **accessibility** — keyboard/focus, semantics, forms, contrast; pair when motion must align with **`prefers-reduced-motion`** and non-color cues (this skill handles **feel**; **accessibility** owns **requirements**).

## Non-Goals

- Owning **Sanity**, **schema**, or **GROQ** unless the change is purely presentational in Studio.
- Replacing **accessibility** (`.agents/skills/accessibility/SKILL.md`) for WCAG-oriented patterns or formal audit claims—this skill covers motion craft, not full a11y coverage.
