 ...
 *
 * @example
 * // Page view (current page)
 * track();
 *
 * @example
 * // Custom payload
 * track({ website: '...', url: '/home', title: 'Home' });
 *
 * @example
 * // Merge with default props
 * track(props => ({ ...props, url: '/home', title: 'Home' }));
 *
 * @example
 * // Custom event
 * track('cta-primary');
 *
 * @example
 * // Event with data (limits: numbers precision 4, strings 500 chars, objects 50 keys)
 * track('cta-primary', { name: 'newsletter', id: 123 });
 */
 ...
 * @example
 * identify('user_123');
 * identify('user_123', { name: 'Bob', email: 'bob@example.com' });
 * identify({ name: 'Bob', email: 'bob@example.com' });
 */
